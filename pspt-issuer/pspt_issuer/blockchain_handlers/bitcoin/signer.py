...
from pspt_issuer.errors import UnverifiedSignatureError, UnableToSignTxError
from pspt_issuer.helpers import to_pycoin_chain
from pspt_issuer.models import Signer
...
def verify_signature(uid, signed_pass_file_name, issuing_address):
    """
    Verify the passport signature matches the expected. Double-check the uid field in the passport and use
    VerifyMessage to confirm that the signature in the passport matches the issuing_address.
    ...
    :param uid:
    :param signed_pass_file_name:
    :param issuing_address:
    :return:
    """

    logging.info('verifying signature for passport with uid=%s:', uid)
    with open(signed_pspt_file_name) as in_file:
        signed_pspt = in_file.read()
        signed_pspt_json = json.loads(signed_pspt)
        to_verify = uid
        signature = signed_pspt_json['signature']
        verified = verify_message(issuing_address, to_verify, signature)
        if not verified:
            error_message = 'There was a problem with the signature for passport uid={}'.format(uid)
