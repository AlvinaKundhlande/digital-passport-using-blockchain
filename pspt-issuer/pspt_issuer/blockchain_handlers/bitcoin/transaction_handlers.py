...
from pspt_issuer.blockchain_handlers.bitcoin import tx_utils
from pspt_issuer.config import ESTIMATE_NUM_INPUTS, V2_NUM_OUTPUTS
from pspt_issuer.errors import InsufficientFundsError
from pspt_issuer.models import TransactionCreator, TransactionHandler
from pspt_issuer.signer import FinalizableSigner


class TransactionV2Creator(TransactionCreator):
    def estimate_cost_for_passport_batch(self, tx_cost_constants, num_inputs=ESTIMATE_NUM_INPUTS):
...
class BitcoinTransactionHandler(TransactionHandler):
    ...
    def ensure_balance(self):
        ...
        transaction_cost = self.transaction_creator.estimate_cost_for_passport_batch(self.tx_cost_constants)
    ...
    def create_transaction(self, op_return_bytes):
        if self.prepared_inputs:
            ...
            cost = self.transaction_creator.estimate_cost_for_passport_batch(self.tx_cost_constants)
            current_total = 0
            inputs = []
