...
from pspt_verifier import IssuerInfo, IssuerKey
from pspt_verifier import TransactionData
from pspt_verifier.errors import *
...
def get_issuer_info(passport_model):
    issuer_json = get_remote_json(passport_model.issuer.id)
    if not issuer_json:
        raise Exception('Issuer URL returned no results ' + passport_model.issuer.id)

    # we use the revocation list in the passport
    revoked_assertions = []
    v2ish = passport_model.version == BlockcertVersion.V2 or passport_model.version == BlockcertVersion.V2_ALPHA
    if v2ish:
        if 'revocationList' in passport_model.passport_json['badge']['issuer']:
            revocation_url = passport_model.passport_json['badge']['issuer']['revocationList']
    ...
    if '@context' in issuer_json:
        ...
        if revoked_assertions:
            # this is a v2 passport with legacy issuer format
