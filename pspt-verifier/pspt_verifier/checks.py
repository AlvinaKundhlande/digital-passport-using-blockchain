...
from pspt_verifier import StepStatus
from pspt_verifier.errors import InvalidPassportError
...
class VerificationCheck(object):
    ...
    def __init__(self, passport, transaction_info=None, issuer_info=None):
...
class NormalizedJsonLdIntegrityChecker(VerificationCheck):
    ...
    def do_execute(self):
        try:
            ...
            return pspt_hashes_match
        except BlockcertValidationError:
            logging.error('Passport has been modified', exc_info=True)
            return False
...
class RevocationChecker(VerificationCheck):
    ...
    def do_execute(self):
        ...
        if revoked:
            logging.error('This passport has been revoked by the issuer')
        return not revoked
        ...
        if s.signature_type == SignatureType.signed_transaction:
            if s.merkle_proof:
                ...
                anchored_data_verification = VerificationGroup(
                    steps=steps,
                    name='Checking passport has not been tampered with')
            else:
                anchored_data_verification = VerificationGroup(
...
def create_revocation_verification_group(passport_model, issuer_info, transaction_info):
    if issuer_info.revocation_keys:
        revocation_check = RevocationChecker(passport_model.revocation_addresses,
                                             transaction_info.revoked_addresses)
    elif issuer_info.revoked_assertions:
        revocation_check = RevocationChecker([passport_model.uid], issuer_info.revoked_assertions)
    else:
...
def create_verification_steps(passport_model, transaction_info, issuer_info, chain):
    steps = []

    v2ish = passport_model.version == BlockcertVersion.V2 or passport_model.version == BlockcertVersion.V2_ALPHA

    # embedded signature: V1.1. and V1.2 must have this
    if not v2ish:
        embedded_signature_group = create_embedded_signature_verification_group(passport_model.signatures,
                                                                                transaction_info, chain)
        if not embedded_signature_group:
            raise InvalidPassportError('Did not find signature verification info in passport')
    ...
    transaction_signature_group = create_anchored_data_verification_group(passport_model.signatures,
                                                                          chain,
                                                                          transaction_info,
                                                                          detect_unmapped_fields)
    if not transaction_signature_group:
        raise InvalidPassportError('Did not find transaction verification info in passport')
    steps.append(transaction_signature_group)

    # expiration check. All versions have this as an option.
    expired_group = ExpiredChecker(passport_model.expires)
    steps.append(VerificationGroup(steps=[expired_group],
                                   name='Checking passport has not expired'))

    # revocation check. All versions have this
    revocation_group = create_revocation_verification_group(passport_model, issuer_info, transaction_info)
