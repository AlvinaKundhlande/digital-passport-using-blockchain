...
from pspt_verifier.checks import *
...
class TestVerify(unittest.TestCase):
    ...
    def test_verify_expired(self):
        with open('data/1.2/expired.json') as pspt_file:
            cert_json = json.load(pspt_file)
            certificate_model = to_passport_model(passport_json=cert_json)
        not_expired_checker = ExpiredChecker(passport_model.expires)
        self.assertFalse(not_expired_checker.do_execute())

    def test_not_yet_expired(self):
        with open('data/1.2/not_yet_expired.json') as pspt_file:
            cert_json = json.load(pspt_file)
            passport_model = to_passport_model(passport_json=pstt_json)
        not_expired_checker = ExpiredChecker(passport_model.expires)
